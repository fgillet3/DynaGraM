...
Bi50 <- tibble(A = rep(.0, 8), B = .0, C = .0, D = .0, E = .0, R = .0, L = .0)
...
# Bi in the vegetation period at year 50
outy50 <- out %>% select(5:11) %>% 
  mutate(year = rep(1:nyears, each = 365)) %>% 
  filter(year == 50) %>% 
  mutate(DOY = 1:365) %>% 
  filter(DOY %in% 91:273) %>% 
  select(-year, -DOY)
Bi50[1, ] <- outy50 %>% summarise_all(mean)
...
outy50 <- out %>% select(5:11) %>% 
  ...
  filter(DOY %in% 91:273) %>% 
  select(-year, -DOY)
Bi50[2, ] <- outy50 %>% summarise_all(mean)
...
outy50 <- out %>% select(5:11) %>% 
  ...
  filter(DOY %in% 91:273) %>% 
  select(-year, -DOY)
Bi50[3, ] <- outy50 %>% summarise_all(mean)
...
outy50 <- out %>% select(5:11) %>% 
  ...
  filter(DOY %in% 91:273) %>% 
  select(-year, -DOY)
Bi50[4, ] <- outy50 %>% summarise_all(mean)
...
outy50 <- out %>% select(5:11) %>% 
  ...
  filter(DOY %in% 91:273) %>% 
  select(-year, -DOY)
Bi50[5, ] <- outy50 %>% summarise_all(mean)
...
outy50 <- out %>% select(5:11) %>% 
  ...
  filter(DOY %in% 91:273) %>% 
  select(-year, -DOY)
Bi50[6, ] <- outy50 %>% summarise_all(mean)
...
outy50 <- out %>% select(5:11) %>% 
  ...
  filter(DOY %in% 91:273) %>% 
  select(-year, -DOY)
Bi50[7, ] <- outy50 %>% summarise_all(mean)
...
outy50 <- out %>% select(5:11) %>% 
  ...
  filter(DOY %in% 91:273) %>% 
  select(-year, -DOY)
Bi50[8, ] <- outy50 %>% summarise_all(mean)

# Convert to long format
Bi50 <- Bi50 %>% mutate(Assemblage = paste0("S", 1:8)) %>% 
